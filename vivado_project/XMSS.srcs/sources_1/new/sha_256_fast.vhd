...
architecture Behavioral of sha_256_fast is
    type state_type is (S_IDLE, S_ROUND, S_LOOP);
    
    constant a_init : unsigned(31 downto 0) := x"489fc27e";
    constant b_init : unsigned(31 downto 0) := x"0a24b1aa";
    constant c_init : unsigned(31 downto 0) := x"f3dd6c3f";
    constant d_init : unsigned(31 downto 0) := x"7ad96290";
    constant e_init : unsigned(31 downto 0) := x"2cab14aa";
    constant f_init : unsigned(31 downto 0) := x"909cf5c9";
    constant g_init : unsigned(31 downto 0) := x"c57b68fb";
    constant h_init : unsigned(31 downto 0) := x"9df1b216";
...
begin
    Message_Schedule : entity work.sha256_message_schedule 
        port map(
            clk     => clk,
            reset => reset,
            d.ctr => r.ctr,
	   ...
	   case r.state is
	     when S_IDLE =>
			...
			if d.enable = '1' then
			     ...
			     v.h1 := h1_init; v.a := a_init;
			     v.h2 := h2_init; v.b := b_init;
			     v.h3 := h3_init; v.c := c_init;
			     v.h4 := h4_init; v.d := d_init;
			     v.h5 := h5_init; v.e := e_init;
			     v.h6 := h6_init; v.f := f_init;
			     v.h7 := h7_init; v.g := g_init;
			     v.h8 := h8_init; v.h := h_init;
